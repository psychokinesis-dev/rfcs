...
## URL 路径重定向
鉴于某些情况下无法修改主机的 DNS ，可以提供一种通过 URL 第一段路径作为域名进行定位的方法。比如对于 URL ： http://127.0.0.1:8181/example.com/test ，即需要将此请求定位到域名 example.com 在 DHT 中指向的地址。注意这个特性需要保留 URL 第一段路径为内部使用。
